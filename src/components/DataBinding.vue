...
// ref()
// 타입 제한 없이 사용할 수 있으나, 프라이미티브(primitive) 타입에 반응적 참조를 위해 사용
// primitive 타입에 좀 더 나은 성능을 제공
  const cnt = ref(0);

// ref 객체의 경우 js에서 접근 시, 해당 값은 ref.value 프로퍼티로 접근
  console.log(cnt.value);


  // 콜백함수 : 호출할 때는 소괄호를 빼고 호출해야 한다.
  function add() {
    cnt.value++;
  }
...
<template>
  ...
  <h1>데이터 바인딩 {{ cnt }}</h1>
  <!-- 데이터 바인딩 :  HTML 요소와 Vue 인스턴스의 데이터를 동적으로 연결하는 기능 -->
...
<!-- ctrl + F5 : 강력 새로고침 : 기존에 다운 받은 걸 싹 다 삭제하고 새로 가져온다. -->
